    ...
    export class GUIClassi extends WUX.WComponent {
        ...
        protected render() {
            ...
            this.btnFind.on('click', (e: JQueryEventObject) => {
                ...
                jrpc.execute('CLASSI.find', [this.fpFilter.getState()], (result) => {
                    ...
                    if (this.selId) {
                        let idx = WUtil.indexOf(result, IClasse.sCLASSE, this.selId);
                        if (idx >= 0) {
                            setTimeout(() => {
            ...
            this.fpFilter.addTextField(IClasse.sCLASSE, 'Classe');
            this.fpFilter.addTextField(IClasse.sDESCRIZIONE, 'Descrizione');

            this.fpDetail = new WUX.WFormPanel(this.subId('fpd'));
            this.fpDetail.addRow();
            this.fpDetail.addTextField(IClasse.sCLASSE, 'Classe');
            ...
            this.btnSave.on('click', (e: JQueryEventObject) => {
                ...
                if (this.isNew) {
                    jrpc.execute('CLASSI.insert', [values], (result) => {
                        this.status = this.iSTATUS_VIEW;

                        this.fpDetail.enabled = false;

                        this.selId = result[IClasse.sCLASSE];
                        this.btnFind.trigger('click');
                    });
                }
                else {
                    jrpc.execute('CLASSI.update', [values], (result) => {
                        this.status = this.iSTATUS_VIEW;
            ...
            this.btnDelete.on('click', (e: JQueryEventObject) => {
                ...
                let id = WUtil.getString(rd[0], IClasse.sCLASSE);
                WUX.confirm(GUI.MSG.CONF_DELETE, (res: any) => {
                    if (!res) return;
                    jrpc.execute('CLASSI.delete', [id], (result) => {
                        this.btnFind.trigger('click');
                    });
                });
            });
            ...
            let rc = [
                ['Classe', IClasse.sCLASSE],
                ['Descrizione', IClasse.sDESCRIZIONE]
            ];
        ...
        protected onSelect(): void {
            ...
            let id = WUtil.getString(item, IClasse.sCLASSE);
            if (!id) return;
            ...
            jrpc.execute('CLASSI.read', [id], (result) => {

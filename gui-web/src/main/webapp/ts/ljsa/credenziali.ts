    ...
    export class GUICredenziali extends WUX.WComponent {
        ...
        protected render() {
            ...
            this.btnFind.on('click', (e: JQueryEventObject) => {
                ...
                jrpc.execute('CREDENZIALI.find', [this.fpFilter.getState(), user.groups], (result) => {
                    ...
                    if (this.selId) {
                        let idx = GUI.indexOf(result, ICredenziale.sID_SERVIZIO, ICredenziale.sID_CREDENZIALE, this.selId);
                        if (idx >= 0) {
                            setTimeout(() => {
            ...
            this.fpFilter.addComponent(ICredenziale.sID_SERVIZIO, 'Servizio', new LJSASelServizi());
            ...
            this.btnSave.on('click', (e: JQueryEventObject) => {
                ...
                if (this.isNew) {
                    jrpc.execute('CREDENZIALI.insert', [values], (result) => {
                        this.status = this.iSTATUS_VIEW;

                        this.fpDetail.enabled = false;

                        this.selId = result[ICredenziale.sID_SERVIZIO] + ":" + result[ICredenziale.sID_CREDENZIALE];
                        this.btnFind.trigger('click');
                    });
                }
                else {
                    jrpc.execute('CREDENZIALI.update', [values], (result) => {
                        this.status = this.iSTATUS_VIEW;
            ...
            this.btnDelete.on('click', (e: JQueryEventObject) => {
                ...
                let ids = WUtil.getString(rd[0], ICredenziale.sID_SERVIZIO);
                let idc = WUtil.getString(rd[0], ICredenziale.sID_CREDENZIALE);
                WUX.confirm(GUI.MSG.CONF_DELETE, (res: any) => {
                    if (!res) return;
                    jrpc.execute('CREDENZIALI.delete', [ids, idc], (result) => {
                        this.btnFind.trigger('click');
                    });
                });
            });
            ...
            let rc = [
                ['Servizio', ICredenziale.sID_SERVIZIO],
                ['Credenziale', ICredenziale.sID_CREDENZIALE],
                ['Email', ICredenziale.sEMAIL]
            ];
        ...
        protected onSelect(): void {
            ...
            let ids = WUtil.getString(item, ICredenziale.sID_SERVIZIO);
            let idc = WUtil.getString(item, ICredenziale.sID_CREDENZIALE);
            if (!ids || !idc) return;
            ...
            jrpc.execute('CREDENZIALI.read', [ids, idc], (result) => {
